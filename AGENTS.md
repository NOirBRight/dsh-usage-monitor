...
- `pnpm run pack:check`：真实 `npm pack` 生成 tarball，校验插件 `exports`/`main`/`types` 目标与归档路径，拒绝 `src/tests/scripts/.env` 等，校验仓库自有 `fixtures/rc1` tarballs/manifest（0.1.5-rc.1 平面）的 tag、commit、registry integrity、版本化父子依赖边（包含重复版本），在全新 pnpm consumer 中仅用本地 tarballs、无效 registry、offline、ignore-scripts、audit/fund disabled、空 `NODE_PATH` 和按父包作用域的 overrides 安装，并执行 Host `import` 与 `window.__ModuleLoader__` 冒烟；不使用动态项目 `node_modules`、`--legacy-peer-deps` 或 omit/force 绕过，任何安装/导入失败直接失败。
