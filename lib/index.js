...
async function readPersistedEvents(persistence, sessionId) {
	return withBudget(READ_BUDGET_MS, async (signal) => {
		...
		if (persistence.readFrom !== void 0) return (await persistence.readFrom(id, SessionLogOffset(0), signal)).events;
		if (persistence.inspect !== void 0) return (await persistence.inspect(id, signal)).events;
		return [];
