...
var UsageProjection = class {
	...
	async query(input) {
		...
		return queryUsage({
			steps: this.readIndexedSteps(input.query, input.workspaces.list()),
			start: input.query.start,
			end: input.query.end,
			pricing: input.pricing ?? BUILTIN_PRICING
		});
	...
	async reconcile(corpus, workspaces, concurrency) {
		...
		try {
			...
			if (this.refreshPromise === next) this.refreshPromise = void 0;
		}
	}
	close() {
	...
	async reconcileNow(corpus, workspaces, concurrency) {
		...
		const results = await mapPool(sessions.filter((session) => session.revision === void 0 || indexed.get(session.id) !== session.revision), Math.max(1, concurrency), async (session) => {
			try {
		...
		const persistentIds = new Set(sessions.filter((session) => session.revision !== void 0).map((session) => session.id));
		const existingIds = this.db.prepare("SELECT id FROM usage_projection_sessions").all().map((row) => row.id);
		const replace = this.db.prepare("INSERT INTO usage_projection_sessions (id, revision, projection_version, complete) VALUES (?, ?, ?, 1)");
		...
		this.db.exec("BEGIN");
		try {
			for (const id of existingIds) if (!persistentIds.has(id)) {
				deleteSteps.run(id);
				deleteSession.run(id);
			}
			for (const result of results) {
				const { session, steps } = result;
				if (session.revision === void 0) continue;
		...
		this.volatile = new Map(results.filter((result) => result.session.revision === void 0 && result.steps !== void 0).map((result) => [result.session.id, result.steps ?? []]));
	}
	readIndexedSteps(query, workspaces) {
		const rows = this.db.prepare("SELECT session_id, time, provider, model, workspace_id, workspace_title, uncached_input_tokens, output_tokens, cache_read_tokens, cache_write_tokens FROM usage_projection_steps WHERE time >= ? AND time < ? ORDER BY time, session_id, ordinal").all(query.start, query.end);
		const steps = [];
		...
		for (const [sessionId, volatileSteps] of this.volatile) {
			...
			if (!this.sessionCanContribute(session, query.end)) continue;
			const workspace = session === void 0 ? void 0 : resolveWorkspace(workspaces, session.id, session.cwd);
			for (const step of volatileSteps) {
				if (step.time < query.start || step.time >= query.end) continue;
				steps.push(workspace === void 0 ? step : {
					...step,
					workspaceId: workspace.id,
					workspaceTitle: workspace.title
				});
...
function apply(ctx) {
	...
	const collect = (query) => {
		...
		const next = (projection === void 0 ? fallback() : projection.query({
			corpus,
			workspaces,
			query,
			concurrency: READ_CONCURRENCY
