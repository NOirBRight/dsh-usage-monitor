...
var UsageProjection = class {
	...
	*/
	...
	async reconcileListing(sessions, request) {
		...
		for (const row of this.db.prepare("SELECT id, revision FROM usage_projection_sessions WHERE projection_version = ?").all(PROJECTION_VERSION)) indexed.set(row.id, row.revision);
		...
		for (let offset = 0; offset < rebuild.length; offset += batchSize) {
			...
			for (const result of results) {
				...
				if (result.error !== void 0) this.onSourceError?.(result.session.id, result.error);
				result.steps = void 0;
				result.error = void 0;
			}
			results.length = 0;
			batch.length = 0;
			if (commitError !== void 0) throw commitError;
		}
		return {
			volatile,
...
*/
function decodeZstdJsonl(buffer) {
...
*/
async function readSessionArtifact(path, signal) {
	const bytes = await readFile(path, signal === void 0 ? void 0 : { signal });
...
*/
function artifactPathFromError(error) {
	if (typeof error !== "object" || error === null || !("location" in error)) return void 0;
	const location = error.location;
	if (typeof location !== "object" || location === null || !("path" in location)) return void 0;
...
async function resolveArtifactPath(persistence, sessionId, signal) {
	if (persistence.resolveCurrentLog === void 0) return void 0;
	return persistence.resolveCurrentLog(SessionId(sessionId), signal);
}
async function withReadHandle(persistence, sessionId, signal, use) {
...
*/
async function loadPersistedFoldSource(persistence, sessionId) {
	return withBudget(READ_BUDGET_MS, async (signal) => {
		...
		if (inspectSession !== void 0) try {
			return {
				kind: "events",
				events: (await inspectSession(id, signal)).events
			};
		} catch (error) {
			const fallback = artifactPathFromError(error);
			if (fallback !== void 0) return {
				kind: "raw",
				content: await readSessionArtifact(fallback, signal)
			};
			throw error;
		}
		if (readRaw === void 0 && inspectSession === void 0 && persistence.resolveCurrentLog === void 0 && typeof persistence.open !== "function") throw new Error(PERSISTENCE_READ_REQUIRED);
		const path = await resolveArtifactPath(persistence, sessionId, signal);
		if (path !== void 0) return {
			kind: "raw",
			content: await readSessionArtifact(path, signal)
		};
		try {
			return await withReadHandle(persistence, sessionId, signal, async (handle) => ({
				kind: "events",
				events: (await handle.read(0, void 0, { signal })).events
			}));
		} catch (error) {
			const fallback = artifactPathFromError(error);
			if (fallback !== void 0) return {
...
async function foldPersistedSession(persistence, stamp) {
	...
	return source.kind === "raw" ? foldRawSessionUsage({
		...stamp,
...
*/
async function resolveRevisionIndex(persistence, signal) {
	const snapshots = await persistence.list({ signal }).catch(() => void 0);
	if (snapshots === void 0) return /* @__PURE__ */ new Map();
...
function apply(ctx, config = DEFAULT_CONFIG) {
	...
	const projection = new UsageProjection(defaultUsageProjectionPath(), { onSourceError: (sessionId, error) => {
	...
	ctx.inject(["connection"], (connectionCtx) => {
		connectionCtx.connection.rpc.handle(USAGE_RPC_CHANNEL, createUsageRpcHandler({ collect }));
	});
}
//#endregion
