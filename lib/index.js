...
var UsageProjection = class {
	...
	refreshRequested = 0;
	refreshCompleted = 0;
	pendingEnd = Number.NEGATIVE_INFINITY;
	latestRequest;
	...
	*/
	async reconcile(request) {
		...
		if (this.workerPromise === void 0) {
			...
			worker.finally(() => {
				if (this.workerPromise === worker) this.workerPromise = void 0;
			}).catch(() => void 0);
		}
		while (this.refreshCompleted < ticket) {
			const worker = this.workerPromise;
			if (worker === void 0) continue;
			await worker;
		}
	}
	...
	close() {
		...
		this.closePromise = (async () => {
			if (this.activeQueries > 0) await new Promise((resolve) => this.idleWaiters.push(resolve));
			this.db.close();
		})();
	...
	async runWorker() {
		while (this.refreshCompleted < this.refreshRequested) {
			...
			if (request === void 0) throw new Error("usage projection worker has no request");
