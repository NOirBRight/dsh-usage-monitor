...
const USAGE_RPC_METHOD = "plugin-rpc/usage-monitor";
...
function createUsageRpcFetch(handler, operator) {
	return async (request) => {
		...
		if (!envelope.success || envelope.data.method !== "plugin-rpc/usage-monitor") return new Response(null, { status: 400 });
		const wrapped = envelope.data.payload;
		if (!isRecord(wrapped) || typeof wrapped.endpoint !== "string") return new Response(null, { status: 400 });
		try {
			...
			if (attachments === void 0 || attachments.length === 0) return Response.json(response);
...
async function readSessionEvents(sessionQuery, sessionId) {
	return withBudget(READ_BUDGET_MS, async () => {
		return (await sessionQuery.readSession(SessionId(sessionId))).events;
	}, "session read timed out");
}
async function foldSessionEvents(sessionQuery, stamp) {
	return foldSessionUsage({
		...stamp,
		events: await readSessionEvents(sessionQuery, stamp.sessionId)
	});
}
/**
