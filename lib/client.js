window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const formatAxisNumber = (value, locale) => {
			...
			if (abs >= 0xe8d4a51000) return `${compact(value / 0xe8d4a51000)}T`;
...
`;
		...
		function UsageDashboard(props) {
			...
			const openMenuRef = (0, react.useRef)(openMenu);
			...
			(0, react.useEffect)(() => {
				...
				const onKey = (event) => {
					if (event.defaultPrevented || event.key !== "Escape" || event.isComposing || event.ctrlKey || event.altKey || event.metaKey || event.shiftKey || openMenuRef.current === null) return;
					event.preventDefault();
					toolbarRef.current?.querySelector("button[aria-expanded=\"true\"]")?.focus();
					setOpenMenu(null);
				};
