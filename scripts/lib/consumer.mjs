...
function runSmoke(plugin) {
  ...
  const hostScript = [
    ...
    'if (typeof host.USAGE_RPC_METHOD !== "string") throw new Error("host RPC method missing");',
    ...
    '};',
    'const context = {',
    '  logger: { warn(message) { throw new Error("unexpected host warning " + message); } },',
    '  get(key) {',
    ...
    '};',
    'host.apply(context);',
    'if (routes.length !== 1 || routes[0].path !== "/api/" + host.USAGE_RPC_METHOD || routes[0].methods.length !== 1 || routes[0].methods[0] !== "POST") throw new Error("host Fetch route was not registered");',
    'const request = (start, end) => new Request("https://example.test" + routes[0].path, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ type: "client-request", rpcId: "usage-smoke", method: host.USAGE_RPC_METHOD, payload: { endpoint: host.USAGE_QUERY_ENDPOINT, payload: { start, end } } }) });',
    'const validResponse = await routes[0].fetch(request(0, 1));',
    'const validEnvelope = await validResponse.json();',
    'if (validResponse.status !== 200 || validEnvelope.type !== "server-response" || validEnvelope.result?.ok !== true || host.decodeUsageSnapshot(validEnvelope.result.value) === undefined) throw new Error("host RPC invariant failed");',
    'const invalidResponse = await routes[0].fetch(request(1, 0));',
    'const invalidEnvelope = await invalidResponse.json();',
    'if (invalidResponse.status !== 200 || invalidEnvelope.type !== "server-response" || invalidEnvelope.result?.ok !== false) throw new Error("host rejected-query invariant failed");',
